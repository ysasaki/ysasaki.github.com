use utf8;
use strict;
use warnings;
use FindBin qw($Bin);
use File::Spec::Functions qw(catdir catfile);
use lib catdir( $Bin, 'lib' );
use DBI;
use SQL::SplitStatement;
use Test::More;

my $dsn = sprintf 'dbi:SQLite:';
my $dbh = DBI->connect( $dsn, '', '',
    { RaiseError => 1, AutoCommit => 1, sqlite_unicode => 1 } )
    or die $DBI::errstr;

setup_db($dbh);

subtest 'lookup artists.id == 1' => sub {

    # primary keyで取得
    my $artist =
        $dbh->selectrow_hashref("SELECT * FROM artists WHERE id = ?", {}, 1);
    isa_ok $artist, 'HASH';
    is $artist->{id},   1,             'id ok';
    is $artist->{name}, 'techno_neko', 'name ok';

    # 付属するCD情報をげっと
    my $cd = $dbh->selectall_arrayref(
        "SELECT * FROM cds WHERE artists_id = ?",
        { Slice => {} },
        $artist->{id}
    );

    subtest 'cds.id == 1' => sub {
        isa_ok $cd->[0], 'HASH';
        is $cd->[0]->{id}, 1, 'id ok';
        is $cd->[0]->{title},
            '初代エルティニスト　スーパーベスト', 'title ok';
    };

    subtest 'cds.id == 2' => sub {
        isa_ok $cd->[1], 'HASH';
        is $cd->[1]->{id}, 2, 'id ok';
        is $cd->[1]->{title}, '根は飽きた', 'title ok';
    };

};

subtest 'search' => sub {
    my $artist = $dbh->selectall_arrayref(
        "SELECT * FROM artists WHERE name = ?",
        { Slice => {} },
        'techno_neko'
    );

    for ( @$artist ) {
        is $_->{name}, 'techno_neko', 'techno_neko!!';
    }
};

subtest 'insert' => sub {
    $dbh->do("INSERT INTO artists (name) VALUES (?)", undef, 'aloelight');

    # insertしたデータの取得
    my $aloelight = $dbh->selectrow_hashref(
        "SELECT * FROM artists WHERE name = ? LIMIT 1",
        {},
        'aloelight'
    );

    isa_ok $aloelight, 'HASH';
    is $aloelight->{id}, 3, 'id ok';
    is $aloelight->{name}, 'aloelight', 'name ok';
};

subtest 'find_or_create' => sub {

    # INSERTするケース
    my $akiym = $dbh->selectrow_hashref(
        "SELECT * FROM artists WHERE name = ? LIMIT 1", {}, 'akiym'
    );

    $akiym ||= do {
        $dbh->do("INSERT INTO artists (name) VALUES (?)", undef, 'akiym');
        $dbh->selectrow_hashref(
            "SELECT * FROM artists WHERE name = ? LIMIT 1", {}, 'akiym'
        );
    };

    isa_ok $akiym, 'HASH';
    is $akiym->{id}, 4, 'id ok';
    is $akiym->{name}, 'akiym', 'name ok';

    # INSERTしないケース
    my $aloelight = $dbh->selectrow_hashref(
        "SELECT * FROM artists WHERE name = ? LIMIT 1", {}, 'aloelight'
    );
    $aloelight ||= do {
        $dbh->do("INSERT INTO artists (name) VALUES (?)", undef, 'aloelight');
        $dbh->selectrow_hashref(
            "SELECT * FROM artists WHERE name = ? LIMIT 1", {}, 'aloelight'
        );
    };

    isa_ok $aloelight, 'HASH';
    cmp_ok $aloelight->{id}, '<', $akiym->{id}, 'aloeligt.id < akiym.id';
};

subtest 'update' => sub {

    # name == techno_nekoのレコードをアップデート
    my $ret = $dbh->do(
        "UPDATE artists SET name = ? WHERE name = ?",
        undef,
        'LT大好き', 'techno_neko'
    );
    ok $ret, 'update techno_neko';

    # updateされたか確認
    my $techno_neko = $dbh->selectrow_hashref(
        "SELECT * FROM artists WHERE id = ?", {}, 1
    );
    is $techno_neko->{name}, 'LT大好き', 'renamed';

    # 別の方法でupdate
    $dbh->do(
        "UPDATE artists SET name = ? WHERE id = ?",
        undef,
        'techno_neko', $techno_neko->{id}
    );

    my $updated_techno_neko = $dbh->selectrow_hashref(
        "SELECT * FROM artists WHERE id = ?", {}, 1
    );
    is $updated_techno_neko->{id}, $techno_neko->{id}, 'id ok';
    is $updated_techno_neko->{name}, 'techno_neko', 'renamed';
};

done_testing;

sub setup_db {
    my $dbh = shift;
    my $stmt = join '', <DATA>;
    my @statements = SQL::SplitStatement->new->split($stmt);
    for (@statements) {
        $dbh->do($_) or die $dbh->errstr;
    }
}

__DATA__
DROP TABLE IF EXISTS artists;
CREATE TABLE artists (
    id integer not null,
    name text not null,
    primary key(id)
);

DROP TABLE IF EXISTS cds;
CREATE TABLE cds (
    id integer not null,
    artists_id integer not null,
    title text not null,
    primary key(id),
    foreign key(artists_id) references artists(id)
);

INSERT INTO artists (name) VALUES ('techno_neko');
INSERT INTO artists (name) VALUES ('onagatani');
INSERT INTO cds (artists_id, title) VALUES (1, '初代エルティニスト　スーパーベスト');
INSERT INTO cds (artists_id, title) VALUES (1, '根は飽きた');
INSERT INTO cds (artists_id, title) VALUES (2, '活イカ到来');

